...
async def test_search_enforces_actor_household_before_querying() -> None:
    session = SearchSession(SimpleNamespace())
...
async def test_search_returns_active_projection_and_escapes_wildcards() -> None:
    ...
    matches = await search_items(session, actor(), household_id, SearchItems("100%_"))
    ...
    assert "items.household_id" in sql
