...
class Events:
    ...
    async def revoke_device(self, workspace_id, device_id, revoked_at) -> None:
        self.calls.append((workspace_id, device_id, revoked_at))
...
async def test_revoke_device_commits_before_publishing_and_is_idempotent() -> None:
    user_id, workspace_id, device_id = uuid4(), uuid4(), uuid4()
    device = SimpleNamespace(
        id=device_id, workspace_id=workspace_id, is_active=True, revoked_at=None
    )
    ...
    assert events.calls == [(workspace_id, device_id, device.revoked_at)]
...
async def test_revoke_device_does_not_publish_when_commit_fails() -> None:
    device = SimpleNamespace(id=uuid4(), workspace_id=uuid4(), is_active=True, revoked_at=None)
    session = DeviceSession(
        device, SimpleNamespace(role=WorkspaceRole.OWNER)
    )
...
async def test_revoke_device_enforces_owner_and_presence() -> None:
    ...
    with pytest.raises(DeviceAccessDenied):
        await revoke_device(
            DeviceSession(device, SimpleNamespace(role=WorkspaceRole.BORROWER)),
            actor(uuid4()),
