...
export function useOverviewInventory(workspaceId: string, token: string, refreshKey = 0): OverviewInventory {
  ...
  useEffect(() => {
    ...
    async function load() {
      const [areas, items, itemPlacements] = await Promise.all([listAreas(token, workspaceId), listItems(token, workspaceId), listItemPlacements(token, workspaceId)])
