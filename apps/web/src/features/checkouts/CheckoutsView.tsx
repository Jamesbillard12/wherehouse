...
type CheckoutView = 'checkout' | 'returns' | 'history' | 'borrowers'
...
export function CheckoutsView({ isOwner, token, workspace }: { isOwner: boolean; token: string; workspace: Workspace }) {
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getCurrentCheckoutSession(token, workspace.id), listCheckoutSessions(token, workspace.id), isOwner ? listBorrowers(token, workspace.id) : Promise.resolve([]), listCheckouts(token, workspace.id, view === 'history' ? 'history' : 'active'), listCheckouts(token, workspace.id, 'active'), isOwner ? listBorrowerInvitations(token, workspace.id) : Promise.resolve([])])
  ...
  }, [isOwner, revision, token, view, workspace.id])
  ...
  const displayedCheckouts = view === 'history' ? checkouts.filter((entry) => entry.returned_at) : checkouts
  const views: { icon: typeof ShoppingCart; id: CheckoutView; label: string }[] = [
    { id: 'checkout', label: 'Checkout', icon: ShoppingCart },
    { id: 'returns', label: 'Returns', icon: RotateCcw },
    { id: 'history', label: 'History', icon: Clock3 },
    ...(isOwner ? [{ id: 'borrowers' as const, label: 'Borrowers', icon: UsersRound }] : []),
  ]
  return <div className="feature-page space-y-6"><PageHeader actions={view === 'checkout' ? <Button onClick={() => setPickerOpen(true)}><Plus />Add items</Button> : view === 'borrowers' ? <Button onClick={() => setBorrowerOpen(true)}><Plus />New borrower</Button> : undefined} description="Check items out, record returns, and manage borrowers." title="Checkouts" />
    <nav aria-label="Checkout sections" className="flex gap-1 overflow-x-auto border-b" role="tablist">{views.map(({ icon: Icon, id, label }) => <button aria-selected={view === id} className={`flex items-center gap-2 border-b-2 px-4 py-3 text-sm font-medium transition-colors ${view === id ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`} key={id} onClick={() => setView(id)} role="tab" type="button"><Icon className="size-4" />{label}{id === 'checkout' && current.items.length ? <span className="rounded-full bg-primary/10 px-2 py-0.5 text-xs">{current.items.length}</span> : null}{id === 'returns' && activeCheckouts.length ? <span className="rounded-full bg-muted px-2 py-0.5 text-xs">{activeCheckouts.length}</span> : null}</button>)}</nav>
    {view === 'checkout' ? <>{actionError ? <StatusMessage tone="error">{actionError}. Your checkout was not changed; remove the unavailable item and try again.</StatusMessage> : null}
    <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_360px]" role="tabpanel">
    ...
    {view === 'returns' || view === 'history' ? <section className="overflow-hidden rounded-2xl border bg-card shadow-sm" role="tabpanel"><div className="border-b px-6 py-5"><h2 className="font-semibold">{view === 'returns' ? 'Items to return' : 'Checkout history'}</h2><p className="mt-1 text-sm text-muted-foreground">{view === 'returns' ? 'Items that are currently away from home.' : 'A record of completed and returned loans.'}</p></div>
      {displayedCheckouts.length ? <div className="divide-y">{displayedCheckouts.map((entry) => <article className="flex items-center justify-between gap-4 px-6 py-4" key={entry.id}><div className="min-w-0"><div className="flex items-center gap-2"><h3 className="truncate text-sm font-semibold">{entry.item_name}</h3>{entry.overdue && !entry.returned_at ? <span className="rounded-full bg-destructive/10 px-2 py-0.5 text-xs font-medium text-destructive">Overdue</span> : null}</div><p className="mt-1 text-xs text-muted-foreground">{entry.borrower_name} · {entry.returned_at ? `Returned ${new Date(entry.returned_at).toLocaleDateString()}` : entry.due_at ? `Due ${new Date(entry.due_at).toLocaleDateString()}` : 'No due date'}</p></div>{view === 'returns' ? <Button onClick={() => void returnCheckout(token, entry.id).then(() => setRevision((v) => v + 1))} size="sm"><RotateCcw />Return</Button> : null}</article>)}</div> : <EmptyState className="min-h-0 border-0 py-16" description={view === 'returns' ? 'Checked-out items will appear here until they are returned.' : 'Completed checkouts will appear here.'} icon={view === 'returns' ? RotateCcw : Clock3} title={view === 'returns' ? 'Everything is home' : 'No checkout history'} />}
    </section> : null}
    {isOwner && view === 'borrowers' ? <section role="tabpanel"><div className="mb-4"><h2 className="text-lg font-semibold">Borrowers</h2><p className="mt-1 text-sm text-muted-foreground">Manage the people who can borrow items and their self-service access.</p></div>{borrowers.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{borrowers.map((borrower) => { const activeInvite = invitations.find((value) => value.borrower_profile_id === borrower.id); return <article className="rounded-2xl border bg-card p-5 shadow-sm" key={borrower.id}><div className="flex items-start gap-3"><span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary"><UserRound className="size-5" /></span><div className="min-w-0 grow"><strong className="block truncate text-sm">{borrower.display_name}</strong><p className="mt-0.5 truncate text-xs text-muted-foreground">{borrower.access_type === 'managed' ? 'Managed borrower' : 'Self-service borrower'}{borrower.email ? ` · ${borrower.email}` : ''}</p></div><Button onClick={() => setEditingBorrower(borrower)} size="sm" variant="ghost">Edit</Button></div>{activeInvite ? <div className="mt-4 space-y-2"><StatusMessage>Invite expires {new Date(activeInvite.expires_at).toLocaleString()}</StatusMessage><div className="flex gap-2"><Button onClick={() => void revokeBorrowerInvitation(token, workspace.id, activeInvite.id).then(() => setRevision((v) => v + 1))} size="sm" variant="outline">Revoke</Button><Button onClick={() => void createBorrowerInvitation(token, workspace.id, borrower.id).then((value) => { setInvitation(value); setRevision((v) => v + 1) })} size="sm" variant="outline">Reissue</Button></div></div> : borrower.access_type === 'managed' && borrower.email ? <Button className="mt-4" onClick={() => void createBorrowerInvitation(token, workspace.id, borrower.id).then(setInvitation)} size="sm" variant="outline">Enable self-service</Button> : null}</article> })}</div> : <EmptyState className="min-h-0 rounded-2xl border py-16" description="Create a borrower to start checking out items." icon={UsersRound} title="No borrowers yet" />}</section> : null}
