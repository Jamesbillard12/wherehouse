...
export function LocationsScreen({ error, inventory, onAddItem, onChangeLocation, onOpenItem, onRefresh, onSelect, onWriteNfc, selected, syncing }: {
  ...
  return <View style={styles.locationPanel}>
      ...
      {selected && contents ? <>
        ...
        {selectedContainer ? <Pressable accessibilityLabel={`Write or replace NFC tag for ${selectedContainer.name}`} onPress={() => void onWriteNfc(selectedContainer.id)} style={[styles.secondaryButton, styles.locationNfcButton]}><Radio color="#4f46e5" size={18} /><Text style={styles.secondaryButtonText}>Write or replace NFC tag</Text></Pressable> : null}
