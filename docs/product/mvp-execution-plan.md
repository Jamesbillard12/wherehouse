...
| Raspberry Pi deployment | Ready for physical validation | OTA persists disconnect-safe operations, backup/rollback health diagnostics, conservative automatic policy, and staged updater evolution; updater transition and preservation matrix still require Pi evidence |
...
| Checkout/return | Implemented, needs physical validation | Stable borrower identity, persistent per-user multi-device sessions, atomic multi-item completion, audience-filtered realtime conflict propagation, self-service invitation/device claim, history, and web/mobile workflows are implemented; physical QR/NFC and cross-device validation remain |
...
- **Purpose/current state:** model-specific image definitions, embedded ARM64 containers, idempotent
  unique first boot, systemd/Compose, Avahi, browser setup status, storage guards, signed remote
  application OTA with backup-gated host execution and web control, reset confirmation, CI, and
  documentation are implemented. UUID-backed external primary storage and opt-in isolated SMB are
  implemented behind the same host boundary. Image/hardware, storage/NAS, and physical OTA evidence
  are outstanding.
