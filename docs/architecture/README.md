# Architecture

- [Current architecture](current-architecture.md)
- [Frontend component architecture](frontend-component-architecture.md)
- [Future architecture](future-architecture.md)
- [Domain model](domain-model.md)
- [ADR 0010: Workspace domain](adr/0010-workspace-domain.md)
- [Authentication and pairing](authentication.md)
- [Offline and sync](offline-sync.md)
- [Settings and household selection](settings.md)
- [Storage and backup](storage-and-backup.md)
- [External interfaces and optional intelligence](external-integrations.md)
- [Controlled generative UI](generative-ui.md)
- [Spatial architecture](spatial-architecture.md)
- [ADRs](adr/README.md)

Current documents describe implemented shape; future documents constrain evolution. Accepted ADRs
remain authoritative unless explicitly superseded.
